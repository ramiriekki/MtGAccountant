...
export class BinderModifyComponent implements OnInit {
    ...
    constructor(
        ...
        public data: { cards: Card[]; collection: CollectionCard[] },
    ...
    ngAfterViewInit(): void {
        this.leftCards = this.data.cards.slice(
            this.leftIndexes.start,
            this.leftIndexes.end
        );
        this.rightCards = this.data.cards.slice(
            this.rightIndexes.start,
            this.rightIndexes.end
        );
    }

    handleNextOpening(): void {
        if (
            this.rightCards[this.rightCards.length - 1].id !==
            this.data.cards[this.data.cards.length - 1].id
        ) {
